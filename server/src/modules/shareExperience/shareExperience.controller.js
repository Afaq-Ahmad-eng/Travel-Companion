...
import { saveExperience,saveExperienceImages } from "./shareExperience.service.js";
...
const dataToValidate = {
  ...
  images: filteredImages,
    ...
    if (!files || files.length === 0) {
      return res.status(400).json({ success: false, message: "At least one picture is required!" });
    }
    ...
    for (const file of files) {
      const url = await uploadBufferToMega(file.buffer, file.originalname, MEGA_EMAIL, MEGA_PASSWORD,FOLDER_NAME);
    ...
    if (Array.isArray(encryptedPictureUrls) && encryptedPictureUrls.length > 0) {
  ...
  newTripImagesUrls = await saveExperienceImages({
    ...
    return res.status(500).json({ status: err.status, success: false, message: err.message || "Internal Server Error" });
  }
};
